...
export default function LanguagePage({ onBack, onNext, toolType = 'audio' }) {
  ...
  useEffect(() => {
    try {
      ...
      if (savedData) {
        if (savedData.language) setSelectedLanguage(savedData.language);
        if (savedData.simpleLanguage !== undefined) setSimpleLanguage(savedData.simpleLanguage);
        if (savedData.customLanguage) setCustomLanguage(savedData.customLanguage);
      }
    } catch (error) {
