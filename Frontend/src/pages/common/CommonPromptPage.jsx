...
export default function PromptPage({ onBack, toolType = 'audio' }) {
  ...
  useEffect(() => {
    ...
    try {
      // Audio verwendet localStorage, MIDI verwendet sessionStorage
      const storage = toolType === 'midi' ? sessionStorage : localStorage;
      const storageKey = toolType === 'midi' ? 'midiFormData' : 'formData';
      const uploadKey = toolType === 'midi' ? 'midiUploadData' : 'uploadData';
      
      const savedFormData = JSON.parse(storage.getItem(storageKey) || '{}');
      const savedUploadData = JSON.parse(storage.getItem(uploadKey) || '{}');
      
      console.log('Loaded formData:', savedFormData);
      console.log('Loaded uploadData:', savedUploadData);
  ...
  const generateFeedback = async (formData, uploadData) => {
    ...
    const hasUploadData = uploadData && Object.keys(uploadData).length > 0;
    
    if (!hasFormData || !hasUploadData) {
      console.error('Missing data - formData:', hasFormData, 'uploadData:', hasUploadData);
  ...
  const generateMidiFeedback = async (formData, uploadData) => {
    const requestData = {
      ...
      useSimpleLanguage: formData.simpleLanguage || false
    };
  ...
  const downloadAnalysisData = () => {
    ...
    const storage = toolType === 'midi' ? sessionStorage : localStorage;
    const uploadKey = toolType === 'midi' ? 'midiUploadData' : 'uploadData';
    const savedUploadData = uploadData || JSON.parse(storage.getItem(uploadKey) || '{}');
