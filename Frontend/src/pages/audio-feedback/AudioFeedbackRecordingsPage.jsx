...
export default function RecordingsPage({ onBack, onNext }) {
  ...
  useEffect(() => {
    // Lade Upload-Daten bevorzugt aus localStorage (Quelle: AudioUpload_Page)
    try {
      const localStored = localStorage.getItem('uploadData');
      const sessionStored = sessionStorage.getItem('uploadData');
      const storedData = localStored || sessionStored;
      if (storedData) {
  ...
  const getAudioUrl = (filename) => {
    const sessionId = uploadData?.sessionId || localStorage.getItem('sessionId') || sessionStorage.getItem('sessionId');
